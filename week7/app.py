...
@app.get("/api/member")
async def search_member(request: Request, username: str):
    if request.session.get("signin"):  
        cursor.execute("SELECT id, name, username FROM member WHERE username=%s", (username,))
        result = cursor.fetchone()
        if result:
            data = {"data": {"id": result[0], "name": result[1], "username": result[2]}}
            return data
        return {"data": None}  
    else:
        return {"data": None}  
...
async def update_name(request: Request, body: Name):
    if request.session.get("signin"):  
        member_id = request.session.get("ID")
